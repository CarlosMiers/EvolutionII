...
public class numeroletras {
    ...
    public String Convertir(String numero, boolean mayusculas, int moneda) {
        ...
        if (Pattern.matches("\\d{1,12},\\d{1,2}", numero)) {
            ...
            if (Long.parseLong(Num[0]) == 0) {// si el valor es cero
                literal = "cero ";
            } else if (Long.parseLong(Num[0]) > 999999999) {// si es mil millones
                literal = getMilMillones(Num[0]);
            } else if (Integer.parseInt(Num[0]) > 999999) {// si es millón
    ...
    private String getMilMillones(String numero) { //000 000 000 000
        // se obtiene los millones
        String millones = numero.substring(numero.length() - 9);
        // se obtiene los mil millones
        String milMillon = numero.substring(0, numero.length() - 9);
        String n = "";
        if (Integer.parseInt(milMillon) > 1) {
            n = getCentenas(milMillon) + "mil  ";
        } else {
            n = getUnidades(milMillon) + "mil ";
        }
