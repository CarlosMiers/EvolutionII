...
import DAO.GenerarBalanceImpositivoDAO;
 ...
 * @author hp
 */
public class Balance_Impositivo extends javax.swing.JFrame {

    Conexion con = null;
    Statement stm = null;
    Tablas modeloBalanceImpositivo = new Tablas();
    ...
    long totalResultado = 0;
    String representante = "";
    String empresaContador = "";
    String rucContador = "";

    public Balance_Impositivo() {
        initComponents();
    ...
    private void cargarTitulo() {
        modeloBalanceImpositivo.addColumn("CUENTA");
        modeloBalanceImpositivo.addColumn("DESCRIPCION");
        modeloBalanceImpositivo.addColumn("DEBITO");
        modeloBalanceImpositivo.addColumn("CREDITO");
        modeloBalanceImpositivo.addColumn("DEUDOR");
        modeloBalanceImpositivo.addColumn("ACREEDOR");
        modeloBalanceImpositivo.addColumn("ACTIVO");
        modeloBalanceImpositivo.addColumn("PASIVO Y P. NETO");
        modeloBalanceImpositivo.addColumn("PERDIDAS");
        modeloBalanceImpositivo.addColumn("GANANCIAS");
        int[] anchos = {100, 300, 120, 120, 120, 120, 120, 120, 120, 120};
        for (int i = 0; i < modeloBalanceImpositivo.getColumnCount(); i++) {
    ...
    private void initComponents() {
        ...
        jTBuscarSucursal.setText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "ventas.jTBuscarClientes.text")); // NOI18N
        ...
        AceptarSuc.setText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "ventas.Aceptarcliente.text")); // NOI18N
        ...
        SalirSuc.setText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "ventas.SalirCliente.text")); // NOI18N
        ...
        labelMetric1.setText("Balance Impositivo");
        ...
        generar.setText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "libroventaconsolidado.jButton1.text")); // NOI18N
        generar.setToolTipText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "libroventaconsolidado.jButton1.toolTipText")); // NOI18N
        ...
        listar.setText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "libroventaconsolidado.jButton2.text")); // NOI18N
        listar.setToolTipText(org.openide.util.NbBundle.getMessage(Balance_Impositivo.class, "libroventaconsolidado.jButton2.toolTipText")); // NOI18N
        ...
        jTable1.setModel(modeloBalanceImpositivo);
    ...
    private void listarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_listarActionPerformed
        GenerarBalanceGeneral Generar = new GenerarBalanceGeneral();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Balance_Impositivo().setVisible(true);
            }
        });
    }
    ...
    private class GenerarConsulta extends Thread {
        ...
        public void run() {
            ...
            int cantidadRegistro = modeloBalanceImpositivo.getRowCount();
            for (int i = 1; i <= cantidadRegistro; i++) {
                modeloBalanceImpositivo.removeRow(0);
            }
            GenerarBalanceImpositivoDAO DAO = new GenerarBalanceImpositivoDAO();
            
            HashMap datos = DAO.generarBalanceImpositivo(Suc, FechaI, FechaF);
            balanceImpositivo = (ArrayList<BalanceImpositivo>) datos.get("balanceImpositivo");
            totalDebitos = (long) datos.get("totalDebitos");
            totalCreditos = (long) datos.get("totalCreditos");
            totalDeudor = (long) datos.get("totalDeudor");
            totalAcreedor = (long) datos.get("totalAcreedor");
            totalActivo = (long) datos.get("totalActivo");
            totalPasivoPN = (long) datos.get("totalPasivoPN");
            totalPerdidas = (long) datos.get("totalPerdidas");
            totalGanancias = (long) datos.get("totalGanancias");
            representante = (String) datos.get("representante");
            empresaContador = (String) datos.get("empresaContador");
            rucContador = (String) datos.get("rucContador");
            for (Iterator it = balanceImpositivo.iterator(); it.hasNext();) {
                BalanceImpositivo bi = BalanceImpositivo.class.cast(it.next());
                String Datos[] = {String.valueOf(bi.getCuenta()), bi.getDescripcion(), formatea.format(bi.getDebitos()), formatea.format(bi.getCreditos()), formatea.format(bi.getDeudor()), formatea.format(bi.getAcreedor()), formatea.format(bi.getActivo()), formatea.format(bi.getPasivo_pn()), formatea.format(bi.getPerdidas()), formatea.format(bi.getGanancias())};
                modeloBalanceImpositivo.addRow(Datos);
            }
            String Datos[] = {"", "TOTALES GENERALES", formatea.format(totalDebitos), formatea.format(totalCreditos), formatea.format(totalDeudor), formatea.format(totalAcreedor), formatea.format(totalActivo), formatea.format(totalPasivoPN), formatea.format(totalPerdidas), formatea.format(totalGanancias)};
            modeloBalanceImpositivo.addRow(Datos);
            jTable1.setRowSorter(new TableRowSorter(modeloBalanceImpositivo));
    ...
    private class GenerarBalanceGeneral extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                parameters.put("empresaContador", empresaContador);
                parameters.put("rucContador", rucContador);
                parameters.put("nombresucursal", nombresucursal.getText());
                parameters.put("FechaInicio", FechaInicio);
                parameters.put("FechaFinal", FechaFinal);
                parameters.put("resultadoPerdidas", jTable1.getValueAt(jTable1.getRowCount() - 1, 8).toString());
                parameters.put("resultadoGanancias",  jTable1.getValueAt(jTable1.getRowCount() - 1, 9).toString());
                parameters.put("totalDebitos", totalDebitos);
                parameters.put("totalCreditos", totalCreditos);
                parameters.put("totalDeudor", totalDeudor);
                parameters.put("totalAcreedor", totalAcreedor);
                parameters.put("totalActivo", totalActivo);
                parameters.put("totalPasivoPN", totalPasivoPN);
                parameters.put("totalPerdidas", totalPerdidas);
                parameters.put("totalGanancias", totalGanancias);

                JasperReport jr = null;

                URL url = getClass().getClassLoader().getResource("Reports/Balance_Impositivo.jasper");
                jr = (JasperReport) JRLoader.loadObject(url);
                JasperPrint masterPrint = null;
                //Se le incluye el parametro con el nombre parameters porque asi lo definimos
                masterPrint = JasperFillManager.fillReport(jr, parameters, new JRBeanCollectionDataSource(balanceImpositivo));
