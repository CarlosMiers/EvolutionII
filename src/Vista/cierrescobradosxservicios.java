...
import Modelo.CierreCobranzaxServicio;
 ...
 * @author hp
 */
public class cierrescobradosxservicios extends javax.swing.JFrame {
     ...
     */
    public cierrescobradosxservicios() {
        initComponents();
    ...
    private void cargarTitulo() {
        ...
        modelo.addColumn("Rebotado");

        int[] anchos = {90, 150, 150, 90, 90, 100, 100, 100, 100};
    ...
    private void initComponents() {
        ...
        setTitle("Ordenes de Crédito Emitidas");
        ...
        etiquetasaldos.setText("Cobranzas Realizadas por Giraduría - Servicio");
                    ...
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(jPanel2Layout.createSequentialGroup()
                                ...
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                .addComponent(giraduria, javax.swing.GroupLayout.PREFERRED_SIZE, 41, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addGap(28, 28, 28)
    ...
    private void buscarcomprobanteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buscarcomprobanteActionPerformed
        ...
        try {
            ...
            if (cm.getCodigo() == 0) {
                GrillaServicio grillacm = new GrillaServicio();
    ...
    private class GenerarConsultaOCredito extends Thread {
        ...
        public void run() {
            ...
            Double Saldo;
            String Cuota;
            String cCasa = "Sin Datos";
            //Antes de Cargar el Jtable Ajustamos el ancho de las columnas con el Ancho que se nos antoje
            int cantidadRegistro = modelo.getRowCount();
            for (int i = 1; i <= cantidadRegistro; i++) {
                modelo.removeRow(0);
            }
            cierre_cobranzaDAO DAO = new cierre_cobranzaDAO();
            try {
                for (cierre_cobranza orden : DAO.MostrarxGiraduriaxVence(Integer.valueOf(giraduria.getText()), FechaI, FechaF, Integer.valueOf(comprobante.getText()))) {
                    if (orden.getCobrado() != null) {
                        Saldo = orden.getEnviado() - orden.getCobrado();
                    } else {
                        Saldo = orden.getEnviado();
                    }

                    Cuota = String.valueOf(orden.getCuota()) + "/" + String.valueOf(orden.getNumerocuota());
                    if(orden.getComercial().getNombre()==null){
                        cCasa="Sin Datos";
                    }else{
                        cCasa=orden.getComercial().getNombre();
                    }

                    String Datos[] = {String.valueOf(orden.getCliente().getRuc()), orden.getCliente().getNombre(), cCasa, formatoFecha.format(orden.getFecha()), formatoFecha.format(orden.getVencimiento()), Cuota, formato.format(orden.getEnviado()), formato.format(orden.getCobrado()), formato.format(Saldo)};
                    modelo.addRow(Datos);
                }
            } catch (SQLException e) {
                JOptionPane.showMessageDialog(null, "ERROR: " + e.getMessage());
    ...
    private class GenerarReporte extends Thread {
        ...
        public void run() {
            ...
            CierreCobranzaxServicio tipo;
            String cEnviado = null;
            String cCobrado = null;
            String cRebotado = null;
            ...
            for (nFila = 0; nFila < jTable1.getRowCount(); nFila++) {
                cEnviado = jTable1.getValueAt(nFila, 6).toString();
                cEnviado = cEnviado.replace(".", "").replace(",", ".");
                
                cCobrado = jTable1.getValueAt(nFila, 7).toString();
                cCobrado = cCobrado.replace(".", "").replace(",", ".");

                cRebotado= jTable1.getValueAt(nFila, 8).toString();
                cRebotado= cRebotado.replace(".", "").replace(",", ".");
                tipo = new CierreCobranzaxServicio(jTable1.getValueAt(nFila, 0).toString(), jTable1.getValueAt(nFila, 1).toString(), jTable1.getValueAt(nFila, 2).toString(), jTable1.getValueAt(nFila, 3).toString(), jTable1.getValueAt(nFila, 4).toString() , jTable1.getValueAt(nFila, 5).toString(), Double.valueOf(cEnviado),Double.valueOf(cCobrado),Double.valueOf(cRebotado));
                Resultados.add(tipo);
            ...
            try {
                ...
                parameters.put("cGiraduria", nombregiraduria.getText());
                parameters.put("cComprobante", nombrecomprobante.getText());

                JasperReport jr = null;

                URL url = getClass().getClassLoader().getResource("Reports/cobranzaxservicio.jasper");
