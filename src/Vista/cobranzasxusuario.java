...
import Modelo.CobranzaxUsuarioReporte;
 ...
 * @author hp
 */
public class cobranzasxusuario extends javax.swing.JFrame {

    Conexion con = null;
    Statement stm = null;
    Tablas modelo = new Tablas();
    Tablas modelousuario = new Tablas();
    ...
    private void cargarTituloTabla() {
        modelo.addColumn("Recibo"); //0
        modelo.addColumn("Fecha");//1
        modelo.addColumn("Cuenta");//2
        modelo.addColumn("Nombre del Cliente");//3
        modelo.addColumn("N° Op.");//4
        modelo.addColumn("Total Pago");//5
        modelo.addColumn("Amortiza");//6
        modelo.addColumn("Int. Ord.");//7
        modelo.addColumn("Gastos");//8
        modelo.addColumn(" Mora");//9
        modelo.addColumn("Punitorio");//10
        modelo.addColumn("Cuota");//11
        modelo.addColumn("Usuario");//12
        modelo.addColumn("Nombre Usuario");//13
        modelo.addColumn("Sucursal");//14
        modelo.addColumn("Moneda");//15
        modelo.addColumn("Comprobante");//16
    ...
    private void TitGir() {
        modelousuario.addColumn("Código");
        modelousuario.addColumn("Nombre");

        int[] anchos = {90, 200};
        for (int i = 0; i < modelousuario.getColumnCount(); i++) {
            tablausuario.getColumnModel().getColumn(i).setPreferredWidth(anchos[i]);
        }
        ((DefaultTableCellRenderer) tablausuario.getTableHeader().getDefaultRenderer()).setHorizontalAlignment(JLabel.CENTER);// Este código es para centrar las cabeceras de la tabla.
        tablausuario.getTableHeader().setFont(new Font("Arial Black", 1, 10));

        // Hacemos Invisible la Celda de Costos de los Productos
        Font font = new Font("Arial", Font.BOLD, 9);
        this.tablausuario.setFont(font);

        DefaultTableCellRenderer TablaRenderer = new DefaultTableCellRenderer();
        TablaRenderer.setHorizontalAlignment(SwingConstants.RIGHT); // aqui defines donde alinear 
        this.tablausuario.getColumnModel().getColumn(0).setCellRenderer(TablaRenderer);
    }
    ...
    private void initComponents() {
        ...
        javax.swing.GroupLayout BuscarUsuarioLayout = new javax.swing.GroupLayout(BuscarUsuario.getContentPane());
        BuscarUsuario.getContentPane().setLayout(BuscarUsuarioLayout);
        BuscarUsuarioLayout.setHorizontalGroup(
            BuscarUsuarioLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(BuscarUsuarioLayout.createSequentialGroup()
                .addComponent(jPanel17, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addGap(0, 0, Short.MAX_VALUE))
            .addComponent(jScrollPane6, javax.swing.GroupLayout.Alignment.TRAILING)
            .addComponent(jPanel18, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
        );
        BuscarUsuarioLayout.setVerticalGroup(
            BuscarUsuarioLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(BuscarUsuarioLayout.createSequentialGroup()
                .addComponent(jPanel17, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        etiquetasaldos.setText("Cobranzas x Usuarios");
        ...
        usuario.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat("#0"))));
    ...
    private void JTBuscarUsuarioKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_JTBuscarUsuarioKeyPressed
        ...
        trsfiltro2 = new TableRowSorter(tablausuario.getModel());
        tablausuario.setRowSorter(trsfiltro2);
        // TODO add your handling code here:
    ...
    private void tablausuarioKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tablausuarioKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            this.Aceptarusuario.doClick();
        }

        // TODO add your handling code here:
    }//GEN-LAST:event_tablausuarioKeyPressed

    private void AceptarusuarioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AceptarusuarioActionPerformed
        int nFila = this.tablausuario.getSelectedRow();
        this.usuario.setText(this.tablausuario.getValueAt(nFila, 0).toString());
        this.nombreusuario.setText(this.tablausuario.getValueAt(nFila, 1).toString());
    ...
    private void BuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BuscarActionPerformed
        ...
        if (usuario.getText().equals("0")) {
            usuario.setText("0");
            nombreusuario.setText("Todos los Usuarios");
            moneda.requestFocus();
        } else {
            usuarioDAO giDAO = new usuarioDAO();
            usuario ve = null;
            try {
                ve = giDAO.buscarId(Integer.valueOf(this.usuario.getText()));
                if (ve.getEmployee_id() == 0) {
                    GrillaUsuario grillagi = new GrillaUsuario();
                    Thread hilogi = new Thread(grillagi);
                    hilogi.start();
                    BuscarUsuario.setModal(true);
                    BuscarUsuario.setSize(482, 575);
                    BuscarUsuario.setLocationRelativeTo(null);
                    BuscarUsuario.setVisible(true);
                    BuscarUsuario.setTitle("Buscar Usuario");
                    BuscarUsuario.setModal(false);
                    this.moneda.requestFocus();
                } else {
                    nombreusuario.setText(ve.getFirst_name());
    ...
    private class GenerarConsultaOCredito extends Thread {
        ...
        public void run() {
            int nUsua = Integer.valueOf(usuario.getText());
            ...
            try {
                for (cobranza cobro : DAO.MostrarxFechaxUsuario(FechaI, FechaF, nUsua, nMoneda)) {
                    String Datos[] = {formatosinpunto.format(cobro.getNumero()),
                        formatoFecha.format(cobro.getFecha()),
                        formatosinpunto.format(cobro.getCliente().getCodigo()),
                        cobro.getCliente().getNombre(),
                        ...
                        String.valueOf(cobro.getCodusuario().getEmployee_id()),
                        cobro.getCodusuario().getFirst_name(),
    ...
    private class GrillaUsuario extends Thread {

        public void run() {

            //Antes de Cargar el Jtable Ajustamos el ancho de las columnas con el Ancho que se nos antoje
            int cantidadRegistro = modelousuario.getRowCount();
            for (int i = 1; i <= cantidadRegistro; i++) {
                modelousuario.removeRow(0);
            }
            usuarioDAO DAOCA = new usuarioDAO();
            try {
                for (usuario ca : DAOCA.todos()) {
                    String Datos[] = {String.valueOf(ca.getEmployee_id()), ca.getFirst_name()};
            ...
            tablausuario.setRowSorter(new TableRowSorter(modelousuario));
            int cantFilas = tablausuario.getRowCount();
        }
    }
    ...
    private class GenerarReporteCobranza extends Thread {
        ...
        public void run() {
            ...
            for (nFila = 0; nFila < jTable1.getRowCount(); nFila++) {
                ...
                System.out.println("cuota " + cCuota);
                datos = new CobranzaxUsuarioReporte(jTable1.getValueAt(nFila, 0).toString(),
                        ...
                        Double.valueOf(cTotalPago),
                        Double.valueOf(cAmortiza),
                        Double.valueOf(cMinteres),
                        Double.valueOf(cGastos),
                        Double.valueOf(cMora),
                        Double.valueOf(cPunitorio),
                        cCuota,
                        jTable1.getValueAt(nFila, 12).toString(),
                        jTable1.getValueAt(nFila, 13).toString(),
                        jTable1.getValueAt(nFila, 14).toString(),
                        jTable1.getValueAt(nFila, 15).toString(),
                        jTable1.getValueAt(nFila, 16).toString());
            ...
            try {
                ...
                URL url = getClass().getClassLoader().getResource("Reports/cobranzaxusuario.jasper");
