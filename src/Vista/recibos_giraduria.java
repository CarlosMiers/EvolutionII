 ...
 * @author hp
 */
public class recibos_giraduria extends javax.swing.JFrame {
            ...
            try {
                if (origendatos.isSelected()) {
                    for (gestioncobro ct : DAO.ReciboCierre(FechaI, FechaF, nGiraduria)) {
                        String Datos[] = {String.valueOf(ct.getCliente().getCodigo()), ct.getCliente().getNombre(), formato.format(ct.getCobrado())};
                        modelo.addRow(Datos);
                    }
                } else {
                    for (gestioncobro ct : DAO.ReciboGestion(FechaI, FechaF, nGiraduria)) {
                        String Datos[] = {String.valueOf(ct.getCliente().getCodigo()), ct.getCliente().getNombre(), formato.format(ct.getCobrado())};
                        modelo.addRow(Datos);
                    }
                }
        ...
        public void run() {
            ...
            for (int i = Integer.valueOf(registro1.getText()); i <= Integer.valueOf(registro2.getText()); i++) {
                ...
                try {
                    ...
                    parameters.put("nGiraduria", nGiraduria);
