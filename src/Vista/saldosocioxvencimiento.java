...
import Modelo.SaldoxSocioReporte;
 ...
 * @author hp
 */
public class saldosocioxvencimiento extends javax.swing.JFrame {
    ...
    private void cargarTitulo() {
        ...
        int[] anchos = {90, 200, 100, 100, 100, 130, 130, 130, 90, 150, 150};
    ...
    private void initComponents() {
        ...
        jTBuscarCliente.setText(org.openide.util.NbBundle.getMessage(saldosocioxvencimiento.class, "ventas.jTBuscarClientes.text")); // NOI18N
        ...
        AceptarCli.setText(org.openide.util.NbBundle.getMessage(saldosocioxvencimiento.class, "ventas.Aceptarcliente.text")); // NOI18N
        ...
        SalirCli.setText(org.openide.util.NbBundle.getMessage(saldosocioxvencimiento.class, "ventas.SalirCliente.text")); // NOI18N
        ...
        etiquetasaldoxsocio.setText("Saldos del Socio al Vencimiento- Resumido");
    ...
    private void generarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_generarActionPerformed
        ...
        try {
            for (cuenta_clientes ct : DAO.MostrarSaldoxSocioVencimiento(nCliente, FechaI)) {
                String Datos[] = {String.valueOf(ct.getCliente().getCodigo()),
                    ...
                    formato.format(ct.getImporte()),
                    formato.format(ct.getSaldo()),
                    formato.format(ct.getPagos()),
                    formatosinpunto.format(ct.getCuota()) + "/" + formatosinpunto.format(ct.getNumerocuota()),
...
///"Reports/estadofinancierosociovencimiento.jasper"
    private class GenerarReporte extends Thread {
        ...
        public void run() {
            ...
            SaldoxSocioReporte tipo;
            String cImporte = "0";
            String cSaldo = "0";
            String cTotalDeuda = "0";

            for (nFila = 0; nFila < jTable1.getRowCount(); nFila++) {

                if (!jTable1.getValueAt(nFila, 5).toString().equals("0")) {
                    cImporte = jTable1.getValueAt(nFila, 5).toString();
                    cImporte = cImporte.replace(".", "").replace(",", ".");
                }

                if (!jTable1.getValueAt(nFila, 6).toString().equals("0")) {
                    cSaldo = jTable1.getValueAt(nFila, 6).toString();
                    cSaldo = cSaldo.replace(".", "").replace(",", ".");
                }
                if (!jTable1.getValueAt(nFila, 7).toString().equals("0")) {
                    cTotalDeuda = jTable1.getValueAt(nFila, 7).toString();
                    cTotalDeuda = cTotalDeuda.replace(".", "").replace(",", ".");
                }

                tipo = new SaldoxSocioReporte(jTable1.getValueAt(nFila, 2).toString().trim(),
                        jTable1.getValueAt(nFila, 3).toString().trim(),
                        jTable1.getValueAt(nFila, 4).toString().trim(),
                        Double.valueOf(cImporte),
                        Double.valueOf(cSaldo),
                        Double.valueOf(cTotalDeuda),
                        jTable1.getValueAt(nFila, 8).toString().trim(),
                        jTable1.getValueAt(nFila, 9).toString().trim(),
                        jTable1.getValueAt(nFila, 10).toString().trim());
                Resultados.add(tipo);
                cImporte = "0";
                cSaldo = "0";
                cTotalDeuda = "0";
            ...
            try {
                ...
                parameters.put("cCliente", socio.getText());
                parameters.put("FechaI", FechaI);
                parameters.put("Resultados", Resultados);

                JasperReport jr = null;

                URL url = getClass().getClassLoader().getResource("Reports/estadofinancierosociovencimiento.jasper");
